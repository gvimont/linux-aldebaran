...
static int compute_machine_id(void)
{
	...
	if (err) {
		printk(KERN_ERR "Fail to init the hasher\n");
		goto error;
	}

	err = crypto_shash_update(desc, buf, HEAD_ID_LEN);
	if (err) {
		printk(KERN_ERR "Fail to update the hasher\n");
		goto error;
	}

	err = crypto_shash_final(desc, hashval);
	if (err) {
